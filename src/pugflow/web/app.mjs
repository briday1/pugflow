...
function renderInspector() {
  ...
  if (nodes.length === selections.length) {
    ...
    if (nodes.length > 1) {
      ...
      inspectorContent.querySelector("[data-node-type]")?.closest("label")?.insertAdjacentHTML("afterend", `<label class="inspector-switch"><span>Hidden</span><input data-node-hidden type="checkbox"${nodes.every((item) => item.hidden) ? " checked" : ""}></label>`);
    ...
    inspectorContent.querySelector("[data-node-type]")?.closest("label")?.insertAdjacentHTML("afterend", `<label class="inspector-switch"><span>Hidden</span><input data-node-hidden type="checkbox"${node.hidden ? " checked" : ""}></label>`);
